...
def cal_start_height(container, start_vol):
    ...
    current_vol = start_vol - vol_conical_tip    # vol in straight part
    max_vol = ((1/3) * math.pi * (height_tube - height_conical_tip) *
            (radius_top2**2 + radius_top2*radius_top + radius_top**2))
    radius_top = (
            (radius_top2*(max_vol - current_vol))+
            (radius_top*(max_vol)))/max_vol
    start_height = (
            height_conical_tip +        # current_height = height conical part 
            (current_vol /             # + height cylindrical part
            ((1/3) * math.pi * 
            (radius_top2**2 + radius_top2*radius_top + radius_top**2)))
            )  
    return start_height
...
def volume_tracking(container, current_vol, aspiration_vol, current_height):
    ...
    radius_top = diameter_top / 2         #radius of the top of the tube in mm
    radius_top2 = diameter_top2 / 2       #radius of the tipstart in mm
    radius_tip = diameter_tip / 2       #radius of the tip of the tube in mm
    vol_conical_tip = ((1/3) * math.pi * height_conical_tip *
                    (radius_tip**2 + radius_tip*radius_top2 + radius_top2**2))
    ## How much volume fills up the conical tip: v = (1/3)*π*h*(r²+r*R+R²)  ##
    ...
    ## The entire tube is build up of 2 truncated cone shapes with different##
    ## dimensions (h = v / ((1/3)*π*(r²+r*R+R²)))                           ##
    ## If liquid level is below vol_conical_tip the delta_height is based on##
    ## R = radius_top2, r = radius_tip
    ...
    ## R = radius_top, r = radius_top2    
    else:
        cylinder_height = height_tube - height_conical_tip
        radius_top = (
            (radius_top2*(cylinder_height - current_height))+
            (radius_top*(cylinder_height)))/height_tube
          ## The radius_top decreases with each pipetting step, so we       ##
          ## calculate a new radius_top for each step.                      ##
        delta_height = (
            aspiration_vol /
