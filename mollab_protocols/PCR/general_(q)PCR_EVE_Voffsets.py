...
Updates:
(SV) 220718: 
    - added offsets parts that are needed for the new robot update (API V2.12)
    - removed a double in the labware, PCR strips was there 3 times,
    one time outside the "if simulate - else" loop, and 2 times within (
    if simulate and else), removed the one outside the loop
"""
    ...
    if simulate: #Simulator
        if mastermix_tube_type == 'tube_5mL': 
            ...
            mastermix_tube = protocol.load_labware_from_definition( 
                ...
                1,                         
                'mastermix_tube')  
            labwares[mastermix_tube_type] = '5mL_screw_cap'
        if sample_tube_type == 'PCR_strip':
