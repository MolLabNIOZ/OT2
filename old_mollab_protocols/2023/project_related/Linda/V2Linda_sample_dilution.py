...
In this version made for the LAZ group I changed the location of the racks a 
bit, and to save pipettetips it can now make 2 aliquots at once.
...
You have to provide:
    ...
    number_of_aliquots
        How many identical aliquots do you want
...
total_dilution_racks = dilution_racks * number_of_aliquots
  ## How many dilution_racks are needed (1,2,3 or 4)
...
# =============================================================================
    ...
    if dilution_tubes == 'plate_96':
        ...
        if dilution_racks >= 2:            
            dilution_dest_2 = protocol.load_labware(
                ...
                6,
                'dilution_dest_2')
        if total_dilution_racks >= 3:             
            dilution_dest_3 = protocol.load_labware(
                'biorad_96_wellplate_200ul_pcr',
                2,
                'dilution_dest_3')
        if total_dilution_racks >= 4:             
        ...
        if dilution_tubes == 'PCR_strips':
            if total_dilution_racks >= 1:
                dilution_dest_1 = protocol.load_labware(
                    ...
                    3,                                      
                    'dilution_dest_1')                      
            if number_of_aliquots == 2 and total_dilution_racks >= 2:
                dilution_dest_2 = protocol.load_labware(
                    'pcrstrips_96_wellplate_200ul',         
                    6,                                      
                    'dilution_dest_2')                      
            if number_of_aliquots == 2 or total_dilution_racks >= 3:    
                dilution_dest_3 = protocol.load_labware(
                    'pcrstrips_96_wellplate_200ul',         
                    2,                                      
                    'dilution_dest_3')                      
            if number_of_aliquots == 2 and total_dilution_racks == 2 or total_dilution_racks >= 4:        
                dilution_dest_4 = protocol.load_labware(
                    'pcrstrips_96_wellplate_200ul',         
                    5,                                      
                    'dilution_dest_4')                          
    ...
    ##### MB: I think this can be removed... but not completely sure because I
        #don't know why it was her in the first place.
...
# =============================================================================
    ...
    if dilution_tubes == 'PCR_strips':
        ...
        for column in dilution_columns:
         ...
         ##makes a list of all wells in 1,2,3 or 4 plates with PCR strips 
    else:
        if dilution_racks == 1: 
    ...
    if number_of_aliquots == 2:
        second_dilution_wells = []
        if dilution_tubes == 'PCR_strips':
            dilution_columns = []
            if dilution_racks >= 1:
                dilution_columns_3 = ( 
            ...
            for column in dilution_columns:
                for well in column:
                    second_dilution_wells.append(well)
        else:
            if dilution_racks == 1: 
                for well in dilution_dest_3.wells():
                    second_dilution_wells.append(well)
    ...
    if number_of_aliquots == 2:
        second_dilution_wells = second_dilution_wells[:number_of_samples]
...
# DILUTING OR TRANSFERRING SAMPLES=============================================
# =============================================================================
    if number_of_aliquots == 1:
    ...
    if number_of_aliquots == 2:
        for (sample_well, dilution_well, second_dilution_well, sample_vol, 
             water_vol) in zip(
                sample_wells, dilution_wells, second_dilution_wells, 
                sample_volumes, water_volumes):
            ## Combine each sample with a dilution_well and a destination well  
            if (sample_vol + 2) * 2 > 17:
                sample_pipette = p300
            else:
                sample_pipette = p20
            sample_pipette.pick_up_tip()
            ...
            sample_pipette.aspirate(sample_vol, sample_well)
              ## aspirate sample_volume_dil = volume for dilution from sample
            sample_pipette.aspirate(30, sample_well.top())
              ## air gap
            sample_pipette.aspirate(sample_vol, sample_well)
              ## aspirate for second aliquot
            sample_pipette.aspirate(5, sample_well.top())
              ## air gap
            
            sample_pipette.dispense(sample_vol + 20, dilution_well)
              ## dispense sample_volume_dil = volume for dilution into dil_well 
            sample_pipette.dispense(200, second_dilution_well)
              ## dispense sample_volume_dil = volume for dilution into dil_well
              
            if water_vol > 0:
                sample_pipette.mix(3, sample_vol + 3, dilution_well)
              ## pipette up&down 3x to get everything from the tip              
            sample_pipette.dispense(20, second_dilution_well)
              ## instead of blow-out
            sample_pipette.drop_tip()
# =============================================================================