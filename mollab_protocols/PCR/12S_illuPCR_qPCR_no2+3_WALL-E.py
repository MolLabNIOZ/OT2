...
# ==================make a list for odd primer destinations==================== 
    primer_dest_1 = []
      ## create an empty list for the first group of primer destinations    ##
    primer_dest_1_columns = (
        [plate_96.columns_by_name()[column_name] for column_name in 
         ['1', '3', '5', '7', '9']]
        )
      ## make a list of columns for the destionation wells, = list of lists! ##
    for column in primer_dest_1_columns:
        for well in column:
            primer_dest_1.append(well)
      ## separate the columns into wells and append them to the emty list   ##
    primer_dest_1_wells = (
        [plate_96.wells_by_name()[well_name] for well_name in 
         ['G11', 'H11']]
        )
      ## make a list of the remaining wells                                 ##
    for well in primer_dest_1_wells:
        primer_dest_1.append(well)
      ## append the wells to the list                                       ##
# ==================make a list for even primer destinations=================== 
    primer_dest_2 = []
      ## create an empty list for the first group of primer destinations    ##
    primer_dest_2_columns = (
        [plate_96.columns_by_name()[column_name] for column_name in 
         ['2', '4', '6', '8', '10']]
        )
      ## make a list of columns for the destionation wells, = list of lists! ##
    for column in primer_dest_2_columns:
        for well in column:
            primer_dest_2.append(well)
      ## separate the columns into wells and append them to the emty list   ##
    primer_dest_2_wells = (
        [plate_96.wells_by_name()[well_name] for well_name in 
         ['G12', 'H12']]
      ...
      ## make a list of the remaining wells                                 ##
    for well in primer_dest_2_wells:
        primer_dest_2.append(well)
      ## append the wells to the list                                       ##
...
# ==============adding primers to 1st half of the sample wells=================    
    ...
    for primer_tube, mix_tube in zip(primers, primer_dest_1):
        p20.pick_up_tip()
...
# ==============adding primers to 2nd half of the sample wells=================    
    ## For the columns in both the source (primers) and the destination     ##
    ## (mix): loop trough the wells in those columns.                       ##
    for primer_tube, mix_tube in zip(primers, primer_dest_2):
        p20.pick_up_tip()
        p20.aspirate(primer_vol, primer_tube)
        primer_mix_vol = primer_vol + 3
        ## primer_mix_vol = volume for pipetting up and down                ##
